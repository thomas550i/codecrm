...
const { document: deal, triggerOnBeforeCreate } = useDocument('CRM Opportunity')
...
const tabs = createResource({
  ...
  cache: ['QuickEntry', 'CRM Opportunity'],
  params: { doctype: 'CRM Opportunity', type: 'Quick Entry' },
  auto: true,
  transform: (_tabs) => {
...
async function createDeal() {
  ...
  createResource({
    url: 'crm.fcrm.doctype.crm_opportunity.crm_opportunity.create_opportunity',
    params: { args: deal.doc },
    auto: true,
    validate() {
      error.value = null
      if (!deal.doc.deal) {
        error.value = __('Deal is required')
        return error.value
      }
    ...
    onSuccess(name) {
      ...
      show.value = false
      router.push({ name: 'Opportunity', params: { dealId: name } })
    },
    onError(err) {
      isDealCreating.value = false
      if (!err.messages) {
...
function openQuickEntryModal() {
  ...
  quickEntryProps.value = { doctype: 'CRM Opportunity' }
